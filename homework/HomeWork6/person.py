...
class Person(object):
    ...
    def attack(self, d):
        d.blood -= self.fight
        print('{}被{}攻击,{}当前血量为{}'.format(d.name,self.name,d.name,d.blood))

    def minattack(self, d):
        self.fight -= 2
        if (self.fight < 0):
            self.fight = 0
        print('{}被{}攻击,{}当前攻击力为{}'.format(self.name,d.name,self.name,self.fight))

    @classmethod
    def bloodjudge(cls):
        flag = 0
        for i in Person.person_list:
            if (i.blood == 0):
                flag += 1
        if (flag == len(Person.person_list)):
            return True
        else:
            return False
